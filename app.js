  ...
  POLICIES.forEach(pol => {
    ...
    row.innerHTML = `
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="text-xs text-teal-400 mt-0.5">${pol.source} · <span class="text-slate-500">Activates year ${pol.year}</span></div>
      </div>`;
    el.appendChild(row);
  });
}
...
function pickLen(val) {
  ...
  document.querySelectorAll('.len-btn').forEach(btn => {
    const active = parseInt(btn.dataset.len) === val;
...
function setAuto(on) {
  ...
  bM.className = 'px-4 py-2 rounded-xl text-sm font-bold transition-all ' + (!on ? 'bg-teal-600 text-white' : 'text-slate-400 hover:text-white');
  bA.className = 'px-4 py-2 rounded-xl text-sm font-bold transition-all ' + ( on ? 'bg-teal-600 text-white' : 'text-slate-400 hover:text-white');
  sc.classList.toggle('hidden', !on);
  bn.classList.toggle('hidden',  on);
...
function setSpeed(ms) {
  autoDelay = ms;
  [['sp1',4000],['sp2',2000],['sp3',800]].forEach(([id, spd]) => {
    document.getElementById(id).className = 'px-3 py-2 rounded-xl text-xs font-bold transition-all '
      + (ms === spd ? 'bg-teal-600 text-white' : 'text-slate-400 hover:text-white');
  });
  if (autoPlay) {
...
function advanceYear() {
  ...
  if (s.year >= simLen) {
    ...
    nb.className   = nb.className.replace('from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700', 'from-slate-700 to-slate-700 cursor-default');
    showFinal();
  }
}
...
function organicGrowth() {
  ...
  let gdpG = 2.5 + polCount * 0.35 + (s.eduBoost && s.year > 10 ? 1.5 : 0);
...
function showYearSummary(events, policies) {
  ...
  let html  = `<div class="text-sm font-bold text-teal-300">${calYear()} — Year ${s.year} Summary</div>`;

  if (events.length === 0 && policies.length === 0) {
...
function buildActivePoliciesList() {
  ...
  const count = document.getElementById('activePolicyCount');
  const active = POLICIES.filter(p => s.activePolicies.has(p.id));
...
function updateWorldComparison() {
  const live = { name:'🏝️ Varde', gdp: s.gdp, happy: Math.round(s.happy), live: true };
  const all  = [...WORLD_COMPARISON.map(c => ({ ...c })), live]
...
function updateDisplay() {
  ...
  const nb = document.getElementById('btnNext');
  if (!s.finished) nb.innerHTML = `▶ Next Year → <span class="text-teal-200 text-sm font-normal">${calYear() + 1}</span>`;
}
    ...
    charts[id] = new Chart(document.getElementById(id), {
      ...
      data: {
        ...
        datasets: [{ data:[], borderColor:color, borderWidth:2,
          fill:true, backgroundColor:color+'18', tension:0.4, pointRadius:0 }]
      },
      options: {
...
function updateCharts() {
  [['chartGDP','gdp'],['chartHappy','happy'],['chartPop','pop']].forEach(([id, key]) => {
...
// ── FINAL ───────────────────────────────────────
function calcGrade() {
  const score = (s.gdp/1000)*0.3 + s.happy*0.4 + s.stability*0.2 + (100-s.debt)*0.1;
  ...
  document.getElementById('finalContent').innerHTML = `
    ...
    <div class="grid grid-cols-2 gap-3">
      <div class="bg-white/5 rounded-2xl p-4 text-center">
        <div class="text-2xl font-black text-emerald-300">${fmtM(s.gdp)}</div>
...
function shareResult() {
  const gr   = calcGrade();
  const text = `🏝️ Republic of Varde — Nation Experiment\n\n`
    + `Grade: ${gr.g} — ${gr.label}\n`
